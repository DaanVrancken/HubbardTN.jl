...
model1 = OB_Sim([1.0],[8.0], 0.0,[0.0],1,1,2.0;spin=true)
...
Q = 1;
bond_dim = 20;
model2 = MB_Sim(t, u, J, P, Q, 2.0, bond_dim; code = name, spin=true);
...
dictionary1 = produce_groundstate(model1; force=Force, path=path);
...
@testset "Excitations" begin 
    ...
    exc = produce_excitations(model1, momenta, nums; charges=[0,0.0,0], force=Force, path=path);
    Es = exc["Es"];
...
@testset "Tools" begin
    N1 = density_state(model1; path=path)
    Nup1, Ndown1 = density_spin(model1; path=path)
